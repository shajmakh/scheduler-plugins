...
func (tm *TopologyMatch) Filter(ctx context.Context, cycleState fwk.CycleState, pod *v1.Pod, nodeInfo fwk.NodeInfo) *fwk.Status {
	...
	if isPreemptionFlow {
		...
		//numalocalities := tm.nrtCache.GetNUMALocalitiesForNode(ctx, nodeName)
		...
		if numaPlacementInfo == nil || numaPlacementInfo.Containers() == 0 {
			...
			nodeTopology = preemption.GetNRTPostPodsEviction(lh, nodeTopology, info, victims, numaPlacementInfo)
		}
	}
